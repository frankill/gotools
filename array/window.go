...
func RowNumber[B ~[]U, C ~[]S, U comparable, S cmp.Ordered](by B, order C) []int {
	...
	for _, v := range group {
		...
		numberid = append(numberid, ArraySeq(0, len(v), 1)...)
	}

	ArraySortByL(ASCInt, numberid, sortid)
...
func MaxValue[B ~[]U, C ~[]S, U comparable, S cmp.Ordered](by B, data C) []S {
	...
	for _, v := range group {
		value = append(value, Rep(ArrayMax(v.First), len(v.First))...)
...
func MinValue[B ~[]U, C ~[]S, U comparable, S cmp.Ordered](by B, data C) []S {
	...
	for _, v := range group {
		value = append(value, Rep(ArrayMin(v.First), len(v.First))...)
		numberid = append(numberid, v.Second...)
	}
...
func FirstValue[B ~[]U, C ~[]S, U comparable, S any](by B, data C) []S {
	...
	for _, v := range group {
		value = append(value, Rep(v.First[0], len(v.First))...)
		numberid = append(numberid, v.Second...)
	}
...
func LastValue[B ~[]U, C ~[]S, U comparable, S any](by B, data C) []S {
	...
	for _, v := range group {
		value = append(value, Rep(v.First[len(v.First)-1], len(v.First))...)
		numberid = append(numberid, v.Second...)
	}
