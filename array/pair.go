...
func PairFromArray[F ~[]T, S ~[]V, T, V any](first F, second S) []Pair[T, V] {
...
func PairFirsts[D ~[]Pair[K, V], K any, V any](data D) []K {
...
// PairSeconds 提取一个包含配对数据的切片中所有第二个元素，返回一个只包含这些第二个元素的切片。
...
func PairSeconds[D ~[]Pair[K, V], K any, V any](data D) []V {
	ld := len(data)
	if ld == 0 {
		return []V{}
	}
...
func PairToMap[D ~[]Pair[K, V], K comparable, V any](data D) map[K][]V {
